...
const EditPic = ({ toggleEditPost, setUser, setIsPPchanging }) => {
  ...
  const deletePic = async () => {
    ...
    try {
      const res = await axios.get(
        "https://insta-clone-r30s.onrender.com/user/delete/profilepic",
        {
          headers: {
  ...
  const changePic = async (file) => {
    ...
    try {
      const res = await axios.put(
        "https://insta-clone-r30s.onrender.com/user/",
        formData,
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  return (
    <div className="bg-white text-black rounded-lg w-80 text-center pt-6 shadow-lg font-roboto">
      ...
      <div
        className="w-full text-blue-500 py-3 border-y cursor-pointer"
        onClick={handleClick}
      >
        <input
          type="file"
